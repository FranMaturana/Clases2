...
### Secuencias:
```{r Secuencias}
seq(1,16) #Me arroja todos los números entre el 1 y el 16
seq(1,16,3) #Lo mismo de arriba pero de 3 en 3
```

### Repeticiones:
```{r Repeticiones} 
rep(1,4) #Que se repita el valor de la izquiera, las veces que dice a la derecha
rep(c(1,5,8,2),4) #Que se repita la lista 4 veces
```

### Ordenar
```{r Ordenar} 
sort(c(1,9,8,4,6,3,6,5,2))
sort(c(1,9,8,4,6,3,6,5,2), decreasing = T) #la T es de true
```
...
Titanic[,.(Survived,Age)]  ## También se puede con nombres de variables de columnas
```

Ahora, aprendamos algunas estrategias de filtrado básico:

```{r filtrado}
Titanic[Survived == 1,]  ## Me quedo con aquellas filas que cumplen la condición de que el individuo sobrevivió, porque el 1 significa True como booleano
...
```{r Viendo base}
View(Titanic) #Me muestra toda la base
head(Titanic,5)  #Alternativa #Me muestra las primras 5 filas de información
nrow(Titanic) #Cuantas filas tiene
ncol(Titanic) #Cuantas columnas tiene la base
```
...
#Me dice que contiene cada una de las columnas, en la base de datos.
```
...
length(unique(Titanic[,Name])) #Cuántas categorías distintas hay.
```
...
Titanic[,quantile(Age, na.rm = T)] #Todo basado en la edad
```

Existe una función que nos puede entregar los datos antes calculados, pero solo ejecutando un comando.

```{r summary}
summary(Titanic) #Nos entrega los datos por columna.
```
...
Titanic[,min(Age, na.rm = T), by = "Survived"] #Me muestra los datos tanto de los sobrevivientes como de los no sobrevivientes
