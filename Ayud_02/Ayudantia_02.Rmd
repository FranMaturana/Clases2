...
```{r str vs. sapply}
str(casos) #Es una previa visualización de la base de datos casos.
sapply(casos, FUN = class) #Me dice que clase es cada columna de la base de datos casos
...
```{r as.Date}
casos$dateRep <- as.Date(casos$dateRep,"GMT") #Transformó dataRep a class a DATE
...
h <- "01"
class(h)
h <- as.numeric(h) #Elimina todos los 0 que pueda tener antes de los numeros reales, asi por ejemplo me queda una columna mucho más facil de leer.
class(h)
...
```{r sapply}
sapply(mas_dias, FUN = unique) #Me da todas las clases de las columnas, el valor no repetido.
```
...
```{r Ejercicio 1: }
sapply(list(casos,muertes,mas_dias),FUN = names) #Nos da las variables de las 3, para ver cuales están en común
```
...
```{r cbind}
#PARA UNIR BASES DE DATOS, QUE TIENE COSAS EN COMUN Y OTRAS QUE LE FALTAN A LA OTRA.
dim(casos)[1]==dim(muertes)[1]# ver si tienen el mismo número de filas.
dim(casos)
dim(muertes)
cbind(casos,muertes[,.(deaths)]) # Para que no se repitan las columnas, nos quedamos solo con la columna "deaths" de "muertes". Tiene todas las columnas de casos mas muerte en muertes.
```
...
```{r merge}
casosymuertes <- merge(casos,muertes, by = c("day","month","year","countriesAndTerritories"), all = T,sort=F)  #Se unen por los datos concatenados para que coincidan las otras columnas, las repetidas.#Mismo año mismo mes, que se ponga la columna de muerte al lado de lo que coincide.
#Si le pusiera ej all.y porque muertes tiene menos variables solo se queda con las que hicieron merge
```
...
View(mas_dias)
sapply(mas_dias, FUN = class)
sapply(casosymuertes, FUN = class) #Me tengo que fijar que las columnas también tengan el mismo class, ya que sino el merge no va a funcionar.
```

Por lo tanto podemos unir las dos bases de datos:

```{r rbind}
al_29_03_2020 <- rbind(casosymuertes,mas_dias) #se usa rbind porque este junta filas para abajo y como lo que nos importa de estas datas es que continuen con las fechas, esta bien.
```
...
#el signo ! antes de complete se refiere a muestrame todas las filas que no estén completas.
...
apply(al_29_03_2020, 2, function(col){
  paste0(round(sum(is.na(col))/length(col),5)*100,"%") #me redondea a 5 decimales, por eso es el 5. Ademas el function (Col) la función se aplica sólo en las columnas, para eso era. El paste % es para que al lado del valor que me de se le pegue el símbolo.
  })
```
...
```{r completa}
al_29_03_2020_completo <- al_29_03_2020[complete.cases(al_29_03_2020),] #Deja solo las filas completas
```
